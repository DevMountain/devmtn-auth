...
class DevmtnStrategy extends Strategy {
  constructor(options, verify) {
    ...
    this._authorizationURL = 'http://localhost:1337/login';
    this._oauth2 = new OAuth2(options.client_token, '', '', this._authorizationURL, '', '');
  }

  authenticate(req, options, next) {
    options = options || {};
    let user = {};
    let self = this;
  ...
  static checkRoles(user, checkRole) {
    hasRole = false;
